...
export function DashboardPage() {
  ...
  const savingsPct = summary && summary.totalIncome > 0
    ? Math.min(Math.round((summary.actualSavings / summary.totalIncome) * 100), 100)
    : 0
    ...
    <div className="px-4 pt-6 pb-4 flex flex-col gap-4">
      ...
      {balanceData ? (
        <Card className="bg-gradient-to-l from-primary-500 to-primary-700 text-white border-0">
          ...
          <p className="text-3xl font-bold mt-1">{formatILS(balanceData.balance)}</p>
        </Card>
      ) : (
        <button
          onClick={() => navigate('/settings/household')}
          className="bg-gradient-to-l from-primary-400 to-primary-600 text-white rounded-2xl p-4 text-start border-0 w-full"
        >
          <p className="text-primary-200 text-sm">יתרה בחשבון</p>
          <p className="text-lg font-semibold mt-1">הגדר יתרה ←</p>
          <p className="text-primary-200 text-xs mt-0.5">לחץ כדי להגדיר את היתרה הנוכחית</p>
        </button>
      )}
      ...
      {summary && (
        <Card>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-lg font-bold ${summary.actualSavings >= 0 ? 'text-success-600' : 'text-danger-600'}`}>
              {formatILS(summary.actualSavings)}
            </span>
          </div>
          <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${summary.actualSavings >= 0 ? 'bg-success-600' : 'bg-danger-600'}`}
              style={{ width: `${Math.max(Math.abs(savingsPct), 2)}%` }}
            />
          </div>
          <div className="grid grid-cols-3 text-xs text-slate-400 mt-2 gap-1">
            <span>הכנסות: <strong className="text-slate-600">{formatILS(summary.totalIncome)}</strong></span>
            <span className="text-center">קבועות: <strong className="text-slate-600">{formatILS(summary.totalFixed)}</strong></span>
            <span className="text-end">משתנות: <strong className="text-slate-600">{formatILS(summary.totalDynamic)}</strong></span>
          </div>
          <div className="mt-2 pt-2 border-t border-slate-50 text-xs text-slate-400 flex justify-between items-center">
            <span>חיסכון צפוי עד סוף החודש</span>
            <strong className={summary.projectedSavings >= 0 ? 'text-success-600' : 'text-danger-600'}>
              {formatILS(summary.projectedSavings)}
            </strong>
          </div>
          {summary.totalIncome === 0 && (
            ...
            >
              + הוסף משכורת כדי לראות חיסכון צפוי
            </button>
          )}
        </Card>
      )}
            ...
            <div className="flex flex-col gap-1.5 flex-1 min-w-0">
              {chartData.map((c) => (
                <div key={c.name} className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-medium text-slate-700 flex-shrink-0 ms-1">{formatILS(c.total)}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              <div key={tx.id} className="flex items-center justify-between py-2.5">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-slate-800 truncate">{tx.note || tx.category?.name || 'הוצאה'}</p>
